...
from metatcr.visualization.dataset_vis import visualize_metavec_inset, calc_metrics, diff_score
import numpy as np
import pandas as pd
...
parser.add_argument('--input_file', type=str, default='./results/data_analysis/datasets_mtx/Wang2022.pk', help='')
parser.add_argument('--out_dir', type=str, default='./results/data_analysis/Wang2022_intra', help='Output directory for processed data')
parser.add_argument('--centroids', type=str, default='./results/data_analysis/96_best_centers.pk', help='centroids file')
parser.add_argument('--meta_data', type=str, default='./data/Wang2022.csv', help='input metadata file')
...
def show_TCR_freq_in_batches(mtx, id_col = "sample_id", label_col = "Identified batch"):
    maxy = 0.065
    df_meta = pd.read_csv(args.meta_data)
    if label_col == "Identified batch":
        df_meta = df_meta[df_meta["Sample label"]=='Tumor']
...
def show_TCR_freq_in_batches_topk(mtx, id_col = "sample_id", label_col = "Identified batch"):
    maxy = 0.065
    df_meta = pd.read_csv(args.meta_data)
    if label_col == "Identified batch":
        df_meta = df_meta[df_meta["Sample label"]=='Tumor']
...
visualize_metavec_inset(representations, smp_list, df_metadata=df_meta, refdata=None,
                      id_col="sample_id", label_col=["Identified batch", "Sequencing batch", "Sample label"],
                      min_dist=0.05, n_neighbors=20, dim=2, type="Wang2022", out_dir=args.out_dir)
...
df_mata = df_meta.sort_values(by=['Sequencing batch'])
batchA_list = df_meta[df_meta["Identified batch"]=='BatchA']["sample_id"].tolist()
batchB_list = df_meta[df_meta["Identified batch"]=='BatchB']['sample_id'].tolist()
...
plt1 = show_TCR_freq_in_batches(mtx = representations, label_col = "Sample label")
plt.savefig(os.path.join(args.out_dir,"TCR frequency in Wang2022 labels.svg"), dpi=600)

plt2 = show_TCR_freq_in_batches(mtx = representations, label_col = "Identified batch")
plt.savefig(os.path.join(args.out_dir,"TCR frequency in Wang2022 batches.svg"), dpi=600)

plt3 = show_TCR_freq_in_batches_topk(mtx = representations, id_col = "sample_id", label_col = "Identified batch")
plt.savefig(os.path.join(args.out_dir,"TCR frequency in Wang2022 batches top10.svg"), dpi=600)