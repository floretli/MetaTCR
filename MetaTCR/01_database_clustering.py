...
#
# ## select best_k after analysis
X = load_pkfile(os.path.join(args.out_dir, 'all_ref_embedding.pk'))
#
best_k = args.selected_k
# ### re clustering with the best k
labels, centroids, last_iteration_stat = kmeans_clustering(X, best_k)
save_pk(os.path.join(args.out_dir, str(best_k) + '_best_labels.pk'), labels)
save_pk(os.path.join(args.out_dir, str(best_k) + '_best_centers.pk'), centroids)
