...
parser.add_argument('--out_dir', type=str, default='./results_50/data_analysis/Emerson_intra', help='Output directory for processed data')
parser.add_argument('--centroids', type=str, default='./results_50/data_analysis/96_best_centers.pk', help='centroids file')
parser.add_argument('--meta_data', type=str, default='./data/CMVlabels.csv', help='input metadata file')
...
### load data
batch1_path = "results/data_analysis/datasets_meta_mtx/Emerson2017_HIP.pk"
batch2_path = "results/data_analysis/datasets_meta_mtx/Emerson2017_Keck.pk"
...
df_meta['set'] = df_meta['set'].replace({'HIP': 'BatchA', 'Keck': 'BatchB'})
df_meta['label'] = df_meta['label'].replace({'0': 'CMV-', '1': 'CMV+'})

# ### umap
# visualize_one_dataset_valid(representations, smp_list, df_metadata=df_meta, refdata=None,
#                       id_col="smpid", label_col=["label", "set"],
...
batchA_list = df_meta[df_meta["set"]=='BatchA']["smpid"].tolist()
batchB_list = df_meta[df_meta["set"]=='BatchB']['smpid'].tolist()
...
plt1 = show_TCR_freq_in_batches(mtx = representations, id_col = "smpid", label_col = "label")
