...
# Calculate number of epts for each TCR and add to df

# df['ept_num'] = df.groupby('CDR3.beta.aa')['Epitope.peptide'].transform('nunique')
# df.to_csv(os.path.join(args.out_dir, 'McPAS_cluster.csv'))
# print(df)
#
# exit()
...
df = df[df['Pathology'].isin(valid_labels)]

print(label_counts)
max_samples_per_class = 1000
new_labels_series = pd.Series(df['Pathology'].values)
new_metadata = pd.DataFrame()
for label in new_labels_series.unique():
    print("label",label)
    class_samples = df[df['Pathology'] == label]
...
labels = new_metadata['Pathology'].values
# print(new_indices)
print(new_metadata)
...
plt.savefig(os.path.join(args.out_dir, 'pathology_dist_unbalance.png'), dpi=600)
